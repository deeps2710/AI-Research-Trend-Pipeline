"""Read-only warehouse health reporting, reusable outside the CLI."""
import duckdb
from src.config import DEFAULT_DATABASE, RAW_DIRECTORY, PathLike, resolve_path
from src.load.files import discover, file_key, fingerprint


def report_status(database: PathLike = DEFAULT_DATABASE, raw_directory: PathLike = RAW_DIRECTORY) -> int:
    ...
    with duckdb.connect(str(database), read_only=True) as con:
        ...
        if ('ops','pipeline_runs') in tables:
            for row in con.execute("SELECT run_id,status,started_at,finished_at,raw_files_processed,error_stage,error_message,epoch(finished_at-started_at) AS duration_seconds FROM ops.pipeline_runs ORDER BY started_at DESC, run_id LIMIT 10").fetchall():
        ...
        if ('ops','processed_files') in tables:
            ...
            print('Latest submitted file records:',con.execute('SELECT records_loaded FROM ops.processed_files ORDER BY processed_at DESC, file_path LIMIT 5').fetchall())
        ...
        if ('ops','data_quality_results') in tables:
            latest = con.execute('SELECT execution_id,run_id,checked_at FROM ops.data_quality_results ORDER BY checked_at DESC, execution_id LIMIT 1').fetchone()
            if latest:
                ...
                for row in con.execute('SELECT check_name,status,observed_value FROM ops.data_quality_results WHERE '+condition+" AND status<>'PASS' ORDER BY layer,check_name",[identifier]).fetchall():
    ...
    print('Pending raw files:', sum(known.get(file_key(p)) != fingerprint(p) for p in discover(raw_directory)))
    return 0
